...
if [ "$ARCH" = "s390x" ] || [ "$ARCH" = "ppc64le" ]; then
  ...
  gn gen -v "out.gn/$BUILD_ARCH_TYPE" --args="$GN_COMPILER_OPTS is_component_build=false is_debug=false v8_target_cpu=\"$TARGET_ARCH\" target_cpu=\"$TARGET_ARCH\" treat_warnings_as_errors=false v8_enable_backtrace=true $CC_WRAPPER"
